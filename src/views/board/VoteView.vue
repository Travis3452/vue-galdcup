      ...
      <div class="relative z-10 space-y-8">
        ...
        <div v-if="isLoading" class="animate-pulse space-y-8">
          <div class="text-center space-y-4">
            <div class="mx-auto w-14 h-14 bg-slate-200 rounded-2xl"></div>
            <div class="mx-auto w-48 h-8 bg-slate-200 rounded-xl"></div>
            <div class="mx-auto w-64 h-4 bg-slate-100 rounded-full"></div>
          </div>

          <div class="grid grid-cols-2 gap-4 md:gap-6">
            <div v-for="i in 2" :key="i" class="flex flex-col items-center">
              <div class="w-full aspect-square bg-slate-200 rounded-[1.5rem] md:rounded-[2rem]"></div>
              <div class="w-2/3 h-5 bg-slate-200 rounded-lg mt-4"></div>
            </div>
          </div>

          <div class="space-y-3 pt-2">
            <div class="w-full h-16 bg-slate-200 rounded-2xl"></div>
            <div class="mx-auto w-24 h-4 bg-slate-100 rounded-full"></div>
          </div>
        </div>
...
async function fetchVoteSession() {
  ...
  try {
    ...
    // 너무 빨리 로딩이 끝나면 스켈레톤이 깜빡거릴 수 있으므로 0.3초 정도 지연
    setTimeout(() => {
