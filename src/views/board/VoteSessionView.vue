      ...
      <div class="relative z-10 space-y-8">
        ...
        <div v-show="isExpanded" class="transition-all duration-500 origin-top">
          <div class="flex items-center justify-center gap-4 md:gap-12 py-4 flex-wrap" :class="{'grayscale opacity-70': voteStatus === 'UPCOMING'}">
            <template v-for="(opt, idx) in voteSession.options" :key="idx">
              <div class="flex flex-col items-center group w-40 md:w-48">
                ...
                <span class="text-sm font-bold text-indigo-600 mt-2">
                  {{ Math.round(calculatePercentage(opt.count)) }}%
                </span>
              </div>
              
              <div v-if="idx < (voteSession.options?.length || 0) - 1" class="text-4xl md:text-6xl font-black text-red-600 italic mx-2 animate-pulse select-none">
                VS
              </div>
            </template>
          </div>
          ...
          <div class="flex flex-col items-center gap-6 pt-6">
            <button @click="onVoteClick" :disabled="voteStatus !== 'LIVE'" class="w-full md:w-auto px-16 py-5 rounded-2xl transition shadow-xl font-black text-xl transform hover:-translate-y-1 active:scale-95 disabled:hover:translate-y-0 disabled:cursor-not-allowed" :class="[voteStatus === 'LIVE' ? 'bg-indigo-600 text-white shadow-indigo-100 hover:bg-indigo-700' : 'bg-slate-200 text-slate-400 shadow-none']">
              투표 참여하기
            </button>
